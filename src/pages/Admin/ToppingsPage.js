...
const ToppingListItem = styled.li`
  background-color: #fff; padding: 10px 15px; border: 1px solid #eee; border-radius: 6px;
  margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center;
  .topping-info { flex-grow: 1; display: flex; justify-content: space-between; align-items: center; }
  .topping-price { color: #7c3aed; font-weight: 500; }
  .topping-status { font-size: 0.8em; padding: 3px 6px; border-radius: 4px; font-weight: bold; margin-left: 15px;
    &.available { background-color: #dcfce7; color: #166534; }
    &.unavailable { background-color: #fee2e2; color: #991b1b; }
  }
  .topping-actions { display: flex; gap: 8px; align-items: center; }
  .topping-actions button { margin-left: 8px; padding: 5px 8px; font-size: 0.8em; }
`;
...
const ToppingsPage = () => {
  ...
  const handleEditTopping = (topping) => {
    ...
    setToppingNameInput(topping.name);
    setToppingPriceInput(topping.price.toString());
  ...
  const openDeleteConfirmModal = (topping) => {
    setItemToDelete(topping);
  ...
  return (
    <>
      ...
      <ConfirmationModal
        ...
        title="Confirmar Exclusão de Adicional"
