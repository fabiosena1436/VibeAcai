...
const AdminWrapper = styled.div` display: flex; position: relative; min-height: calc(100vh - 70px); `;
...
const StyledNavLink = styled(NavLink)` display: block; color: #e0d6ff; text-decoration: none; padding: 12px 15px; border-radius: 6px; margin-bottom: 8px; transition: background-color 0.2s; &:hover { background-color: rgba(255, 255, 255, 0.1); } &.active { background-color: #7c3aed; color: #fff; font-weight: bold; } `;
...
const MenuButton = styled.button` display: none; background: none; border: none; cursor: pointer; z-index: 1100; @media (max-width: 768px) { display: block; position: fixed; top: 80px; right: 15px; background-color: #7c3aed; color: white; border-radius: 50%; width: 50px; height: 50px; font-size: 20px; box-shadow: 0 4px 8px rgba(0,0,0,0.2); } `;
    ...
    <AdminWrapper>
      <Sidebar isOpen={isSidebarOpen}>
        ...
        <NavList>
          ...
          <li onClick={closeSidebar}><StyledNavLink to="/admin/sizes">Tamanhos dos copos</StyledNavLink></li> {/* <-- ADICIONAR O NOVO LINK AQUI */}
