...
const ReportsSection = styled.div`
  background-color: #fff; padding: 20px; border-radius: 8px; margin-bottom: 40px; 
  box-shadow: 0 2px 5px rgba(0,0,0,0.07);
  display: grid; grid-template-columns: 1fr 1fr; gap: 20px 40px; 
  h3 { margin-top: 0; color: #555; font-size: 1.2em; margin-bottom: 15px; grid-column: 1 / -1; }
  div { padding-top: 15px; border-top: 1px solid #eee; } 
  p { font-size: 1.1em; color: #333; margin: 10px 0; strong { color: #7c3aed; } }
`;
...
const DashboardOverviewPage = () => {
  ...
  // Estado para a aba de status selecionada
  ...
  const handleUpdateOrderStatus = async (orderId, newStatus) => {
    ...
    try {
      ...
      // Para o relatório se atualizar, precisamos recalcular
      // A forma mais fácil é chamar fetchData() novamente
      fetchData();
  ...
  // Definição das colunas para a tabela DataGrid
  const columns = [
    ...
    { 
      ...
      headerName: 'Data', 
      width: 170, 
      // CORREÇÃO AQUI: Adicionada verificação extra para params.row
      valueGetter: (params) => {
        if (!params.row || !params.row.createdAt) {
          return null;
        }
        return params.row.createdAt.toDate();
      },
      renderCell: (params) => params.value ? params.value.toLocaleString('pt-BR') : 'N/A'
    },
    { field: 'customerName', headerName: 'Cliente', width: 200, flex: 1 }, // flex: 1 para ocupar espaço
    { 
      field: 'grandTotal', 
      headerName: 'Total', 
      width: 130, 
      type: 'number',
      renderCell: (params) => `R$ ${params.value?.toFixed(2).replace('.', ',') || '0,00'}`
    },
    { 
      field: 'status', 
      headerName: 'Status', 
  ...
  return (
    <div>
      ...
      {loading ? ( <LoadingText>Calculando relatórios...</LoadingText> ) : (<ReportsSection>
        <h3 style={{gridColumn: '1 / -1'}}>Resumo de Vendas</h3>
      ...
      <Box sx={{ width: '100%', bgcolor: 'background.paper', borderRadius: '8px', boxShadow: 1 }}>
        <Tabs value={selectedStatusTab} onChange={handleTabChange} centered>
        ...
        <Box sx={{ height: 600, width: '100%' }}>
          <DataGrid
            ...
            loading={loading}
            // Adicionei um local text para quando não há linhas a serem exibidas
