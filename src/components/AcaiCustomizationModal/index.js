  ...
  useEffect(() => {
    const fetchModalData = async () => {
      ...
      try {
        ...
        const [toppingsSnapshot, sizesSnapshot] = await Promise.all([ getDocs(toppingsQuery), getDocs(sizesQuery) ]);
        ...
        if (sizesList.length > 0 && !activePromo) {
          setSelectedSize(sizesList[0]);
           ...
           setSelectedSize({ name: 'Promoção', price: activePromo.price }); // Usa o preço da promoção
        }
        
        setSelectedToppings([]);
      } catch (error) { toast.error("Não foi possível carregar as opções."); onClose(); }
      ...
      {loadingData ? <p>A carregar opções...</p> : (
        <>
              ...
              {availableSizes.length > 0 ? (
                availableSizes.map(size => (
                  <SizeButton key={size.id} selected={selectedSize?.id === size.id} onClick={() => setSelectedSize(size)}>
                    {/* AQUI ESTÁ A MUDANÇA: Mostra o nome e o preço total do tamanho */}
          ...
          <ToppingOptionsContainer>
            ...
            {availableToppings.map(topping => { const isPromotional = activePromo && promoToppingIds.includes(topping.id); const isDisabled = isPromotional && limitReached && !selectedToppings.includes(topping.id); return ( <ToppingLabel key={topping.id} className={isDisabled ? 'disabled' : ''}> <div> <input type="checkbox" checked={selectedToppings.includes(topping.id)} onChange={() => handleToppingToggle(topping.id)} disabled={isDisabled} /> <span className="topping-name">{topping.name}</span> </div> <ToppingPriceText className={isPromotional ? 'free' : ''}> {isPromotional ? <strong>Grátis</strong> : `+ R$ ${topping.price.toFixed(2).replace('.', ',')}`} </ToppingPriceText> </ToppingLabel> ); })}
          </ToppingOptionsContainer>
