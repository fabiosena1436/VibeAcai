...
import { ModalOverlay, ModalContent, ModalHeader, ModalCloseButton, OptionsSection, ModalFooter } from './styles';
...
const ToppingOptionsContainer = styled(OptionsSection)` h4 { margin-bottom: 10px; color: #555; } .promo-title { font-size: 1em; color: #7c3aed; font-weight: bold; }`;
...
const AcaiCustomizationModal = ({ isOpen, onClose, productToCustomize }) => {
  ...
  useEffect(() => {
    ...
    if (activePromo && activePromo.price !== undefined) {
      ...
      finalPrice = productToCustomize.price + (selectedSize.priceModifier || 0);
    } else {
      finalPrice = productToCustomize.price;
    }
  ...
  return (
    <ModalOverlay onClick={onClose}>
      <ModalContent onClick={(e) => e.stopPropagation()}>
        ...
        <ModalHeader>Personalize seu {activePromo ? activePromo.target.productName : productToCustomize.name}</ModalHeader>
